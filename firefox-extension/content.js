...
function calcBaseNeeds(base, gamedata) {
  ...
  for (const [bType, orders] of allInfiniteByType) {
    ...
    // Weight each order by its planet-adjusted recipe time so every order gets equal
    // daily throughput. For extraction/farming buildings, lower abundance/fertility means
    // a longer effective cycle time, so more buildings are needed — matching the game's
    // own balancing. For all other building types getPlanetFactor returns 1, so the
    // weight is identical to the old recipe.timeMinutes formula.
    let totalWeightedTime = 0;
    for (const { recipe } of orders) {
      totalWeightedTime += recipe.timeMinutes / getPlanetFactor(recipe, basePlanet, gamedata);
    }
    ...
    for (const [rId, { recipe, count }] of ordersByRId) {
      const effectiveTime = recipe.timeMinutes / getPlanetFactor(recipe, basePlanet, gamedata);
      const weight      = (effectiveTime * count) / totalWeightedTime;
    ...
    if (shipData.length) {
      ...
      if (shipData.length > 1) {
        ...
        for (const m of fleetMatArr) {
          ...
          mLbl.style.cssText = 'color:#9090b0;font-size:11px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;';
